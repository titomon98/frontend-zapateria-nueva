<template>
  <b-container fluid>
    <b-modal id="modal-1-cancelados" ref="modal-1-cancelados" title="Cambio de estado de traslado" size="xl">
      <b-alert
    ...
    <b-modal id="modal-2-cancelados" ref="modal-2-cancelados" title="Traslado individual" size="xl">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Listado de traslados cancelados/anulados</h4>
...
export default {
  ...
  data () {
    return {
      ...
      alertErrorText: '',
      alertVariant: '',
      apiBase: apiUrl + '/traslados/listCancelado',
      options: [
  ...
  methods: {
    ...
    printSale (data) {
      ...
      me.encabezado.anticipo = data.anticipo
      this.$refs['modal-2-cancelados'].show()
    },
    closeModal (action) {
      switch (action) {
        case 'especifico': {
          this.$v.$reset()
          this.$refs['modal-1-cancelados'].hide()
          this.form.id = 0
          this.form.name = ''
          this.form.state = 1
          break
        ...
        case 'ver': {
          this.$v.$reset()
          this.$refs['modal-2-cancelados'].hide()
    ...
    cambiarEstado () {
      ...
      axios
        ...
        .then((response) => {
          me.$refs.vuetable.refresh()
          me.$refs['modal-1-cancelados'].hide()
          me.resetData()
        })
        .catch((error) => {
