<template>
  <b-container fluid>
    <b-modal id="modal-1-en-camino" ref="modal-1-en-camino" title="Cambio de estado de traslado" size="xl">
      <b-alert
    ...
    <b-modal id="modal-2-en-camino" ref="modal-2-en-camino" title="Traslado individual" size="xl">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Listado de traslados en camino</h4>
...
export default {
  ...
  data () {
    return {
      ...
      alertErrorText: '',
      alertVariant: '',
      apiBase: apiUrl + '/traslados/listEnCamino',
      options: [
  ...
  methods: {
    ...
    printSale (data) {
      ...
      me.encabezado.anticipo = data.anticipo
      this.$refs['modal-2-en-camino'].show()
    },
    closeModal (action) {
      switch (action) {
        case 'especifico': {
          this.$v.$reset()
          this.$refs['modal-1-en-camino'].hide()
          this.form.id = 0
          this.form.name = ''
          this.form.state = 1
          break
        ...
        case 'ver': {
          this.$v.$reset()
          this.$refs['modal-2-en-camino'].hide()
    ...
    cambiarEstado () {
      ...
      axios
        ...
        .then((response) => {
          me.$refs.vuetable.refresh()
          me.$refs['modal-1-en-camino'].hide()
          me.resetData()
        })
        .catch((error) => {
