<template>
  <b-container fluid>
    <b-modal id="modal-1-fisica" ref="modal-1-fisica" title="Cambio de estado de traslado" size="xl">
      <b-alert
    ...
    <b-modal id="modal-2-fisica" ref="modal-2-fisica" title="Traslado individual" size="xl">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Listado de traslados incompletos por falta de existencia</h4>
...
export default {
  ...
  data () {
    return {
      ...
      alertErrorText: '',
      alertVariant: '',
      apiBase: apiUrl + '/traslados/listExistenciaFisica',
      options: [
  ...
  methods: {
    ...
    printSale (data) {
      ...
      me.encabezado.anticipo = data.anticipo
      this.$refs['modal-2-fisica'].show()
    },
    closeModal (action) {
      switch (action) {
        case 'especifico': {
          this.$v.$reset()
          this.$refs['modal-1-fisica'].hide()
          this.form.id = 0
          this.form.name = ''
          this.form.state = 1
          break
