      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Mayor</h4>
...
export default {
  ...
  beforeMount () {
    var date = new Date()
    this.selectedDates.start = new Date(date.getFullYear(), date.getMonth(), 1)
    this.selectedDates.end = new Date(date.getFullYear(), date.getMonth(), 1)
    this.getMayor()
  },
  data () {
    return {
      ...
      alertErrorText: '',
      alertVariant: '',
      apiBase: apiUrl + '/libros/mayor/list',
      fields: [
        {
          name: 'descripcion',
  ...
  methods: {
    ...
    EXCEL () {
      ...
      worksheet.addRow(['FECHA', 'DOCUMENTO', '', 'COMPRADOR', '', 'TOTAL FACTURA VENTA/SERVICIO', 'PRECIO BASE', '', 'ISER/VENTAS', 'IVA DÉBITO FISCAL', 'TOTALES'])
      worksheet.addRow(['', 'TIPO', 'NÚMERO', 'NIT O DPI', 'NOMBRE', '', 'VENTA', 'SERVICIO', '', '', ''])
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        link.href = url
        link.download = 'LibroMayor.xlsx'
        link.click()
      })
    },
    async getMayor () {
      let date = new Date()
      let end = moment(this.selectedDates.endDate).format('YYYY-MM-DDThh:mm')
      let oneMonthAgo = new Date(date.getFullYear(), date.getMonth() - 1, date.getDate())
      let start = moment(oneMonthAgo).format('YYYY-MM-DDThh:mm')
      await axios.get(apiUrl + '/detalle_partidas/mayor?start=' + start + '&end=' + end
      ).then((response) => {
