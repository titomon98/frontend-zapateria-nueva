<template>
    <b-modal ref="doctorModal" title="Seleccione al doctor" size="xl">
      <b-alert
  ...
  methods: {
    ...
    rowClicked (dataItem, event) {
      ...
      if (!this.selectedItems.includes(itemId) && this.selectedItems.length === 0) {
        ...
        this.doctor = dataItem
        this.$emit('doctor', this.doctor)
        ...
        this.showAlertError()
        this.alertErrorText = 'Solo puede seleccionar un doctor'
      }
    },
    showAlertError () {
    ...
    setDoctor () {
      if (this.doctorEdit) {
        this.doctor = this.doctorEdit
        this.selectedItems.push(this.doctorEdit.id)
      } else {
        this.patient = []
        this.selectedItems = []
      }
