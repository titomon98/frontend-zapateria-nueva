<template>
  <b-container fluid>
    ...
    <b-modal id="modal-1-supplies-laboratory" ref="modal-1-supplies-laboratory" title="Agregar insumos">
      <b-alert
      ...
      <b-form>
        <b-form-group label="Tipo de Insumo:">
          ...
          <div v-if="$v.form.type.$error" class="invalid-feedback-vselect">
            Debe seleccionar el tipo de insumo
          </div>
        </b-form-group>
    ...
    <b-modal id="modal-2-supplies-laboratory" ref="modal-2-supplies-laboratory" title="Editar Insumos">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
    ...
    <b-modal id="modal-3-supplies-laboratory" ref="modal-3-supplies-laboratory" title="Desactivar Insumo">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
      ...
      <h6 class="my-4">
        ¿Desea desactivar el insumo: {{ form.name }} ?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
          variant="primary"
          @click="onState()
                  $bvModal.hide('modal-3-supplies-laboratory')"
          >Desactivar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-3-supplies-laboratory')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
    <b-modal id="modal-4-supplies-laboratory" ref="modal-4-supplies-laboratory" title="Activar Insumo">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
      ...
      <h6 class="my-4">
        ¿Desea activar el insumo: {{ form.name }} ?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
          variant="primary"
          @click="onState()
                  $bvModal.hide('modal-4-supplies-laboratory')"
          >Activar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-4-supplies-laboratory')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Insumos del Laboratorio</h4>
            ...
            <b-button variant="primary"  v-b-modal.modal-1-supplies-laboratory>AGREGAR NUEVO</b-button>
          </template>
              ...
              <template slot="actions" slot-scope="props">
                <b-button-group>
                  <b-button
                    ...
                    v-b-modal.modal-2-supplies-laboratory
                    class="mb-2"
                    size="sm"
                  ...
                  <b-button
                    ...
                    @click="
                      ...
                      props.rowData.estado == 1
                        ? $bvModal.show('modal-3-supplies-laboratory')
                        : $bvModal.show('modal-4-supplies-laboratory');
                    "
                    class="mb-2"
                    size="sm"
                    :variant="
  ...
  data () {
    return {
      ...
      fields: [
        ...
        {
          name: 'tipo_insumo_laboratorio.nombre',
          sortField: 'tipo_insumo_laboratorio.nombre',
  ...
  methods: {
    ...
    closeModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-supplies-laboratory'].hide()
          this.form.id = 0
          this.form.number = ''
          this.form.state = 1
          this.form.site = []
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-supplies-laboratory'].hide()
          this.form.id = 0
          this.form.name = ''
          this.form.code = ''
          this.form.type = []
    ...
    onSave () {
      const me = this
      axios.post(apiUrl + '/insumo_laboratorio/create', {
        form: me.form })
        .then((response) => {
      ...
      if (this.form.state === 1) {
        axios
          ...
          .then((response) => {
            ...
            me.$refs.vuetable.refresh()
            me.$refs['modal-3-supplies-laboratory'].hide()
          })
          .catch((error) => {
            me.alertVariant = 'danger'
            me.showAlertError()
        ...
        axios
          ...
          .then((response) => {
            ...
            me.$refs.vuetable.refresh()
            me.$refs['modal-4-supplies-laboratory'].hide()
          })
          .catch((error) => {
            me.alertVariant = 'danger'
            me.showAlertError()
