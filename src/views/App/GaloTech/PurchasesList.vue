...
export default {
  ...
  methods: {
    ...
    cierreDiario () {
      ...
      doc.text('No.', 2, altura)
      doc.text('Nit', 3, altura)
      doc.text('Proveedor', 4, altura)
      doc.text('Contribuyente', 8, altura)
      doc.text('Área de destino', 14, altura)
      doc.text('Total', 18, altura)
      altura = altura + 1
      doc.setFontSize(10).setFont(undefined, 'normal')
      for (let i = 0; i < me.arrayToday.length; i++) {
        doc.text(me.arrayToday[i].id.toString(), 2, altura)
        doc.text(me.arrayToday[i].nit, 3, altura)
        doc.text(me.arrayToday[i].proveedore.nombre, 4, altura)
        doc.text(me.arrayToday[i].contribuyente.nombre, 8, altura)
        doc.text(me.arrayToday[i].destino.nombre, 14, altura)
        doc.text(me.arrayToday[i].total, 18, altura)
        altura = altura + 0.5
        if (altura > 25) {
          doc.addPage()
          altura = 2
      ...
      doc.save('CierreDeHoyCompras.pdf')
    }
  }
}
</script>
