<template>
  <b-container fluid>
    ...
    <b-modal id="modal-1-medicine" ref="modal-1-medicine" title="Agregar medicamento">
      <b-alert
    ...
    <b-modal id="modal-2-medicine" ref="modal-2-medicine" title="Editar Medicamento">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
    ...
    <b-modal id="modal-3-medicine" ref="modal-3-medicine" title="Desactivar medicamento">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
    ...
    <b-modal id="modal-4-medicine" ref="modal-4-medicine" title="Activar medicamento">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Medicamentos</h4>
            ...
            <b-button variant="primary"  v-b-modal.modal-1-medicine>AGREGAR NUEVO</b-button>
          </template>
                  ...
                  <b-button
                    ...
                    @click="
                      ...
                      props.rowData.estado == 1
                        ? $bvModal.show('modal-3-medicine')
                        : $bvModal.show('modal-4-medicine');
                    "
                    class="mb-2"
                    size="sm"
                    :variant="
...
export default {
  ...
  data () {
    return {
      ...
      alertErrorText: '',
      alertVariant: '',
      apiBase: apiUrl + '/medicamento/list',
      fields: [
        {
          name: '__slot:actions',
          title: 'Acciones',
  ...
  methods: {
    ...
    closeModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-medicine'].hide()
          this.form.id = 0
          this.form.name = ''
          this.form.code = ''
          this.form.existence = ''
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-medicine'].hide()
          this.form.id = 0
          this.form.name = ''
          this.form.state = 1
    ...
    onSave () {
      const me = this
      axios.post(apiUrl + '/medicamento/create', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'success'
          me.showAlert()
          me.alertText = 'Se ha creado la clinca ' + me.form.name + ' exitosamente'
    ...
    onUpdate () {
      ...
      axios.put(apiUrl + '/medicamento/update', {
        form: me.form })
        .then((response) => {
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        axios
          .put(apiUrl + '/medicamento/deactivate', {
            id: this.form.id
          })
          .then((response) => {
        ...
        axios
          .put(apiUrl + '/medicamento/activate', {
            id: this.form.id
          })
          .then((response) => {
