<template>
  <b-container fluid>
    ...
    <b-modal id="modal-1-inventory" ref="modal-1-inventory" size="xl" title="Agregar al inventario">
      ...
      <b-form>
         <b-form-group label="Medicamento:">
          <v-select
            name="medicine"
            v-model="$v.form.medicine.$model"
            :state="!$v.form.medicine.$error"
            :options="medicines"
            :filterable="false"
          ...
          >
            ...
            <template v-slot:option="option">
              {{ 'Nombre: '+ option.nombre +' Código: '+ option.codigo }}
            </template>
      ...
      <b-form>
         <b-form-group label="Medicamento:">
          ...
          >
            ...
            <template v-slot:option="option">
              {{ 'Nombre: '+ option.nombre +' Código: '+ option.codigo + ' Marca: ' + option.marca.nombre + ' Tipo: ' + option.tipo_medicamento.nombre + ' Presentación: ' + option.presentacione.nombre }}
            </template>
            <template slot="selected-option" slot-scope="option">
              {{ 'Nombre: '+ option.nombre +' Código: '+ option.codigo + ' Marca: ' + option.marca.nombre + ' Tipo: ' + option.tipo_medicamento.nombre + ' Presentación: ' + option.presentacione.nombre }}
            </template>
    ...
    <b-modal id="modal-3-inventory" ref="modal-3-inventory" title="Desactivar del inventario">
      ...
      <h6 class="my-4">
        ¿Desea desactivar del inventario: {{ form.medicine.nombre }} ?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
    ...
    <b-modal id="modal-4-inventory" ref="modal-4-inventory" title="Activar del inventario">
      ...
      <h6 class="my-4">
        ¿Desea activar del inventario: {{ form.medicine.nombre }} ?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
...
export default {
  ...
  data () {
    return {
      ...
      alertErrorText: '',
      alertVariant: '',
      apiBase: apiUrl + '/inventario/list',
      fields: [
        {
          name: '__slot:actions',
          title: 'Acciones',
        ...
        },
        {
          ...
          callback: function (value) {
            return value.nombre + ' | Código: ' + value.codigo + ' | Marca: ' + value.marca.nombre + ' | Tipo: ' + value.tipo_medicamento.nombre + ' | Presentación: ' + value.presentacione.nombre
          },
          width: '20%'
        },
        {
  ...
  methods: {
    ...
    onSave () {
      const me = this
      axios.post(apiUrl + '/inventario/create', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'success'
          me.showAlert()
          me.alertText = 'Se ha creado el inventario ' + me.form.medicine.nombre + ' exitosamente'
    ...
    onUpdate () {
      ...
      axios.put(apiUrl + '/inventario/update', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'primary'
          me.showAlert()
          me.alertText = 'Se ha actualizado el inventario ' + me.form.medicine.nombre + ' exitosamente'
          me.$refs.vuetable.refresh()
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        axios
          .put(apiUrl + '/inventario/deactivate', {
            id: this.form.id
          })
          .then((response) => {
            me.alertVariant = 'warning'
            me.showAlert()
            me.alertText = 'Se ha desactivado el inventario ' + me.form.medicine.nombre + ' exitosamente'
            me.$refs.vuetable.refresh()
        ...
        axios
          .put(apiUrl + '/inventario/activate', {
            id: this.form.id
          })
          .then((response) => {
            me.alertVariant = 'info'
            me.showAlert()
            me.alertText = 'Se ha activado el inventario ' + me.form.medicine.nombre + ' exitosamente'
            me.$refs.vuetable.refresh()
