<template>
  <b-container fluid>
    <b-modal id="modal-1-psicologia" ref="modal-1-psicologia" title="Agregar">
      ...
      <b-form>
        <b-form-group label="Descripción de terapia psicologica:">
          <b-form-textarea
            v-model.trim="$v.form.contenido.$model"
            :state="!$v.form.contenido.$error"
            placeholder="Ingresar la descripción de la terapia psicologica"
          ></b-form-textarea>
          <div v-if="$v.form.contenido.required.$invalid" class="invalid-feedback">
            Debe ingresar la descripción de la terapia psicologica
          </div>
        </b-form-group>
      </b-form>
    ...
    <b-modal id="modal-2-psicologia" ref="modal-2-psicologia" title="Editar">
      <b-form>
        <b-form-group label="Descripción de psicologia:">
          <b-form-textarea
            v-model.trim="$v.form.contenido.$model"
            :state="!$v.form.contenido.$error"
            placeholder="Ingresar la descripción de la terapia psicologica"
          ></b-form-textarea>
          <div v-if="$v.form.contenido.required.$invalid" class="invalid-feedback">
            Debe ingresar la descripción de psicologia
          </div>
        </b-form-group>
      </b-form>
      <template #modal-footer="{}">
    ...
    <b-modal id="modal-3-psicologia" ref="modal-3-psicologia" title="Desactivar">
      <h6 class="my-4">
      ...
      <template #modal-footer="{}">
        <b-button
          ...
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-3-psicologia')"
          >Desactivar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-3-psicologia')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
    <b-modal id="modal-4-psicologia" ref="modal-4-psicologia" title="Activar">
      <h6 class="my-4">
      ...
      <template #modal-footer="{}">
        <b-button
          ...
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-4-psicologia')"
          >Activar</b-button
        >
        <b-button variant="danger" @click="closeModal.hide('modal-4-psicologia')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Psicologia</h4>
                ...
                <b-button
                  ...
                  @click="
                    ...
                    props.rowData.estado == 1
                      ? $bvModal.show('modal-3-psicologia')
                      : $bvModal.show('modal-4-psicologia');
                  "
                  class="mb-2"
                  :variant="
...
export default {
  ...
  mounted () {
    ...
    this.getPsicologia(this.patientId)
  },
  computed: {
    ...mapGetters([
      'currentUser'
  ...
  data: () => {
    return {
      ...
      alertVariant: '',
      data: [],
      apiBase: apiUrl + '/psicologia/list',
      typeOptions: [],
      options: [
  ...
  methods: {
    ...
    generatePDF (data) {
      ...
      doc.text('Instrucciones de terapia psicológica: ', 1.5, 5.5)
      ...
      doc.save('Psicologia.pdf')
    },
    getPsicologia (id) {
      let me = this
      axios.get(apiUrl + `/psicologia/get?id=${id}`)
    ...
    closeModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-psicologia'].hide()
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-psicologia'].hide()
          // Aqui agregar variables
          this.form.id = 0
          this.form.contenido = ''
          this.form.paciente = []
    ...
    openModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-psicologia'].show()
          // Aqui agregar variables
          this.form.id = 0
          this.form.contenido = ''
          this.form.paciente = []
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-psicologia'].show()
          // Aqui agregar variables
          this.form.id = 0
          this.form.contenido = ''
          this.form.paciente = []
    ...
    onSave () {
      ...
      axios.post(apiUrl + '/psicologia/create', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'success'
          me.showAlert()
          me.alertText = 'Se han creado las indicaciones exitosamente'
          me.getPsicologia(this.patientId)
    ...
    onUpdate () {
      const me = this
      axios.put(apiUrl + '/psicologia/update', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'primary'
          me.showAlert()
          me.alertText = 'Se han actualizado las indicaciones exitosamente'
          me.getPsicologia(this.patientId)
          me.$refs.vuetableDiagnosis.refresh()
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        axios
          .put(apiUrl + '/psicologia/deactivate', { // url de detalles
            id: this.form.id
          })
          .then((response) => {
            me.getPsicologia(this.patientId)
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-3-psicologia'].hide()
          })
          .catch((error) => {
        ...
        axios
          .put(apiUrl + '/psicologia/activate', {
            id: this.form.id
          })
          .then((response) => {
            me.getPsicologia(this.patientId)
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-4-psicologia'].hide()
          })
          .catch((error) => {
            console.error('There was an error!', error)
          })
