<template>
  <b-container fluid>
    <b-modal id="modal-1-lenguaje" ref="modal-1-lenguaje" title="Agregar">
      ...
      <b-form>
        <b-form-group label="Descripción de terapia:">
          <b-form-textarea
            ...
            placeholder="Ingresar la descripción de terapia de lenguaje"
          ></b-form-textarea>
    ...
    <b-modal id="modal-2-lenguaje" ref="modal-2-lenguaje" title="Editar">
      <b-form>
        <b-form-group label="Descripción de terapia:">
          <b-form-textarea
            v-model.trim="$v.form.contenido.$model"
            :state="!$v.form.contenido.$error"
            placeholder="Ingresar la descripción de terapia del lenguaje"
          ></b-form-textarea>
          <div v-if="$v.form.contenido.required.$invalid" class="invalid-feedback">
            Debe ingresar la descripción de la terapia
          </div>
    ...
    <b-modal id="modal-3-lenguaje" ref="modal-3-lenguaje" title="Desactivar">
      <h6 class="my-4">
        ¿Desea desactivar el lenguaje para el paciente?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-3-lenguaje')"
          >Desactivar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-3-lenguaje')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
    <b-modal id="modal-4-lenguaje" ref="modal-4-lenguaje" title="Activar">
      <h6 class="my-4">
        ¿Desea activar el lenguaje para el paciente?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-4-lenguaje')"
          >Activar</b-button
        >
        <b-button variant="danger" @click="closeModal.hide('modal-4-lenguaje')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Lenguaje</h4>
                ...
                <b-button
                  ...
                  @click="
                    ...
                    props.rowData.estado == 1
                      ? $bvModal.show('modal-3-lenguaje')
                      : $bvModal.show('modal-4-lenguaje');
                  "
                  class="mb-2"
                  :variant="
...
export default {
  ...
  mounted () {
    ...
    this.getLenguajes(this.patientId)
  },
  computed: {
    ...mapGetters([
      'currentUser'
  ...
  methods: {
    ...
    generatePDF (data) {
      ...
      doc.text('Instrucciones de terapia de lenguaje: ', 1.5, 5.5)
      ...
      doc.save('Lenguaje.pdf')
    },
    getLenguajes (id) {
      let me = this
      axios.get(apiUrl + `/lenguaje/get?id=${id}`)
    ...
    closeModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-lenguaje'].hide()
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-lenguaje'].hide()
          // Aqui agregar variables
          this.form.id = 0
          this.form.contenido = ''
          this.form.paciente = []
    ...
    openModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-lenguaje'].show()
          // Aqui agregar variables
          this.form.id = 0
          this.form.contenido = ''
          this.form.paciente = []
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-lenguaje'].show()
          // Aqui agregar variables
          this.form.id = 0
          this.form.contenido = ''
          this.form.paciente = []
    ...
    onSave () {
      ...
      axios.post(apiUrl + '/lenguaje/create', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'success'
          me.showAlert()
          me.alertText = 'Se ha creado el lenguaje exitosamente'
          me.getLenguajes(this.patientId)
    ...
    onUpdate () {
      const me = this
      axios.put(apiUrl + '/lenguaje/update', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'primary'
          me.showAlert()
          me.alertText = 'Se ha actualizado el lenguaje exitosamente'
          me.getLenguajes(this.patientId)
          me.$refs.vuetableDiagnosis.refresh()
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        axios
          .put(apiUrl + '/lenguaje/deactivate', { // url de detalles
            id: this.form.id
          })
          .then((response) => {
            me.getLenguajes(this.patientId)
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-3-lenguaje'].hide()
          })
          .catch((error) => {
        ...
        axios
          .put(apiUrl + '/lenguaje/activate', {
            id: this.form.id
          })
          .then((response) => {
            me.getLenguajes(this.patientId)
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-4-lenguaje'].hide()
          })
          .catch((error) => {
            console.error('There was an error!', error)
          })
