<template>
  <b-container fluid>
    <b-modal id="modal-1-vitals" ref="modal-1-vitals" title="Agregar">
    ...
    <b-modal id="modal-2-vitals" ref="modal-2-vitals" title="Editar">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
    ...
    <b-modal id="modal-3-vitals" ref="modal-3-vitals" title="Desactivar">
      <h6 class="my-4">
      ...
      <template #modal-footer="{}">
        <b-button
          ...
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-3-vitals')"
          >Desactivar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-3-vitals')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
    <b-modal id="modal-4-vitals" ref="modal-4-vitals" title="Activar">
      <h6 class="my-4">
      ...
      <template #modal-footer="{}">
        <b-button
          ...
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-4-vitals')"
          >Activar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-4-vitals')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Signos Vitales</h4>
            ...
            <b-button variant="primary"  v-b-modal.modal-1-vitals>AGREGAR DIAGNÓSTICO</b-button>
          </template>
                ...
                <b-button
                  @click="
                    ...
                    props.rowData.estado == 1
                      ? $bvModal.show('modal-3-vitals')
                      : $bvModal.show('modal-4-vitals');
                  "
                  class="mb-2"
                  :variant="
...
export default {
  ...
  data: () => {
    return {
      ...
      alertVariant: '',
      data: [],
      apiBase: apiUrl + '/vitales/list',
      typeOptions: [],
      options: [
        { value: '1', nombre: 'Tipo de diagnóstico' },
        { value: '2', nombre: 'Estado' },
        { value: '3', nombre: 'Diagnóstico' }
      ],
      columna: { value: '1', nombre: 'Tipo de diagnóstico' },
      fields: [
        {
          name: '__slot:actions',
          title: 'Acciones',
  ...
  methods: {
    ...
    onSave () {
      const me = this
      axios.post(apiUrl + '/diagnostico/create', {
        form: me.form })
        .then((response) => {
    ...
    onUpdate () {
      const me = this
      axios.put(apiUrl + '/diagnostico/update', {
        form: me.form })
        .then((response) => {
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        ...
        axios
          .put(apiUrl + '/diagnostico/activate', {
            id: this.form.id
          })
          .then((response) => {
            me.getDiagnosis(this.patientId)
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-4-vitals'].hide()
          })
          .catch((error) => {
            console.error('There was an error!', error)
          })
