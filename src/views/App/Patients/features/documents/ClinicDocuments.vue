<template>
  <b-container fluid>
    <b-modal id="modal-1-clinicdocs" ref="modal-1-clinicdocs" title="Agregar">
      ...
      <b-form>
        <b-form-group label="Descripción de documento:">
          <b-form-input
            v-model.trim="$v.form.contenido.$model"
            :state="!$v.form.contenido.$error"
            placeholder="Ingresar la descripción de documento"
          ></b-form-input>
          <div v-if="$v.form.contenido.required.$invalid" class="invalid-feedback">
            Debe ingresar la descripción de documento
          </div>
        </b-form-group>
        <b-form-group
          id="input-group-10"
    ...
    <b-modal id="modal-2-clinicdocs" ref="modal-2-clinicdocs" title="Editar">
      <b-form>
        <b-form-group label="Descripción de documento:">
          <b-form-input
            v-model.trim="$v.form.contenido.$model"
    ...
    <b-modal id="modal-3-clinicdocs" ref="modal-3-clinicdocs" title="Desactivar">
      <h6 class="my-4">
        ¿Desea desactivar el documento para el paciente?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-3-clinicdocs')"
          >Desactivar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-3-clinicdocs')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
    <b-modal id="modal-4-clinicdocs" ref="modal-4-clinicdocs" title="Activar">
      <h6 class="my-4">
        ¿Desea activar el documento para el paciente?
      </h6>
      <template #modal-footer="{}">
        <b-button
          type="submit"
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-4-clinicdocs')"
          >Activar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-4-clinicdocs')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Documentos Clínicos</h4>
            ...
            <b-button variant="primary"  v-b-modal.modal-1-clinicdocs>AGREGAR DOCUMENTO</b-button>
          </template>
                ...
                <b-button
                  @click="
                    ...
                    props.rowData.estado == 1
                      ? $bvModal.show('modal-3-clinicdocs')
                      : $bvModal.show('modal-4-clinicdocs');
                  "
                  class="mb-2"
                  :variant="
...
export default {
  ...
  data: () => {
    return {
      ...
      alertVariant: '',
      data: [],
      apiBase: apiUrl + '/documentos/list',
      typeOptions: [],
      options: [
  ...
  methods: {
    ...
    async sendFile () {
      ...
      try {
        await axios.post(apiUrl + '/documentos/upload', formData)
    ...
    getDocuments (id) {
      let me = this
      axios.get(apiUrl + `/documentos/get?id=${id}`)
    ...
    closeModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-clinicdocs'].hide()
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-clinicdocs'].hide()
          // Aqui agregar variables
          this.form.id = 0
          this.form.contenido = ''
          this.dropzoneVal = false
    ...
    onSave (nombre) {
      ...
      axios.post(apiUrl + '/documentos/create', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'success'
          me.showAlert()
          me.alertText = 'Se ha creado el documento exitosamente'
          me.getDocuments(this.patientId)
    ...
    onUpdate () {
      const me = this
      axios.put(apiUrl + '/documentos/update', {
        form: me.form })
        .then((response) => {
          me.alertVariant = 'primary'
          me.showAlert()
          me.alertText = 'Se ha actualizado el documento exitosamente'
          me.getDocuments(this.patientId)
          me.$refs.vuetableDiagnosis.refresh()
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        axios
          .put(apiUrl + '/documentos/deactivate', { // url de detalles
            id: this.form.id
          })
          .then((response) => {
            me.getDocuments(this.patientId)
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-3-clinicdocs'].hide()
          })
          .catch((error) => {
        ...
        axios
          .put(apiUrl + '/documentos/activate', {
            id: this.form.id
          })
          .then((response) => {
            me.getDocuments(this.patientId)
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-4-clinicdocs'].hide()
          })
          .catch((error) => {
            console.error('There was an error!', error)
          })
