<template>
  <b-container fluid>
    <b-modal id="modal-1-diagnosis" ref="modal-1-diagnosis" title="Agregar">
    ...
    <b-modal id="modal-2-diagnosis" ref="modal-2-diagnosis" title="Editar">
      <b-alert
        :show="alertCountDownError"
        dismissible
        fade
    ...
    <b-modal id="modal-4-diagnosis" ref="modal-4-diagnosis" title="Activar">
      <h6 class="my-4">
      ...
      <template #modal-footer="{}">
        <b-button
          ...
          variant="primary"
          @click="onState()
          $bvModal.hide('modal-4-diagnosis')"
          >Activar</b-button
        >
        <b-button variant="danger" @click="$bvModal.hide('modal-4-diagnosis')"
          >Cancelar</b-button
        >
      </template>
    </b-modal>
      ...
      <b-col md="12">
        <iq-card>
            <template v-slot:headerTitle>
              <h4 class="card-title mt-3">Diagnósticos</h4>
            ...
            <b-button variant="primary"  v-b-modal.modal-1-diagnosis>AGREGAR DIAGNÓSTICO</b-button>
          </template>
...
export default {
  ...
  data: () => {
    return {
      ...
      typeOptions: [],
      options: [
        { value: 'tipo', nombre: 'Tipo de diagnóstico' },
        { value: 'estado_diagnostico', nombre: 'Estado' },
        { value: 'descripcion', nombre: 'Diagnóstico' }
      ],
      columna: { value: 'tipo', nombre: 'Tipo de diagnóstico' },
      fields: [
        {
          name: '__slot:actions',
          title: 'Acciones',
  ...
  methods: {
    ...
    closeModal (action) {
      switch (action) {
        case 'save': {
          this.$v.$reset()
          this.$refs['modal-1-diagnosis'].hide()
        ...
        case 'update': {
          this.$v.$reset()
          this.$refs['modal-2-diagnosis'].hide()
          // Aqui agregar variables
          this.form.id = 0
          this.form.tipo.nombre = ''
          this.form.estado_diagnostico.nombre = ''
    ...
    onState () {
      ...
      if (this.form.state === 1) {
        ...
        axios
          ...
          .then((response) => {
            me.$refs.vuetableDiagnosis.refresh()
            me.$refs['modal-4-diagnosis'].hide()
          })
          .catch((error) => {
            console.error('There was an error!', error)
          })
